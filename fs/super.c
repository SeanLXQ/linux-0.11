...
static struct super_block * read_super(int dev)
{
	...
	*((struct d_super_block *) s) =//将缓冲区中的超级块复制 super_block[8]中的第一项
	...
	if (s->s_magic != SUPER_MAGIC) {//判断超级块的魔数是否正确
		s->s_dev = 0;
		free_super(s);//释放超级块
		return NULL;
	}
	...
	for (i=0;i<Z_MAP_SLOTS;i++)
		s->s_zmap[i] = NULL;
	block=2;
	for (i=0 ; i < s->s_imap_blocks ; i++)     //虚拟盘的第一块是超级块，第二块开始时i节点位图和逻辑块
		if ((s->s_imap[i]=bread(dev,block))) //位图，把虚拟盘上i节点位图所占的所有逻辑块读到缓冲区，
			block++;                           //分别挂接到s_imap[8]上
		else
			break;
	for (i=0 ; i < s->s_zmap_blocks ; i++)     //把虚拟盘上逻辑位图所占用的所有逻辑块
...
void mount_root(void)
{
    ...
    // 并等待按键。
	for(i=0;i<NR_FILE;i++)
		file_table[i].f_count=0; //初始化file_table[64]为后续程序做准备         // 初始化文件表
	if (MAJOR(ROOT_DEV) == 2) {// 2代表软盘，此时跟设备是虚拟盘，是1，反之，没有虚拟盘，则加载软盘
